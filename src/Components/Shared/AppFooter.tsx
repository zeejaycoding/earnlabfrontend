...
function SocialBtn({ href, label, children, white }: { href: string; label: string; children: React.ReactNode; white?: boolean }) {
  return (
    <a
      href={href}
      target="_blank"
      ...
      className={`w-10 h-10 rounded-[10px] border flex items-center justify-center transition-all ${
        white ? "text-white bg-[#1E2133] border-[#111324] hover:bg-[#252840]" : "bg-[#1C2033] border-[#2A2D3E] text-[#8C9DB6] hover:text-white hover:bg-[#252840]"
      }`}
    >
      {children}
    </a>
...
const SocialIcons = ({ white }: { white?: boolean }) => (
  <div className="flex items-center gap-3">
    <SocialBtn href={X_URL} label="X (Twitter)" white={white}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
      </svg>
    </SocialBtn>
    <SocialBtn href={TELEGRAM_URL} label="Telegram" white={white}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <line x1="22" y1="2" x2="11" y2="13" />
        <polygon points="22 2 15 22 11 13 2 9 22 2" />
      </svg>
    </SocialBtn>
    <SocialBtn href={DISCORD_URL} label="Discord" white={white}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
...
const LangThemeBtns = ({
  ...
  <div className="flex items-center gap-2">
    {/* Language Selector */}
    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${whiteIcons ? "bg-transparent border-transparent" : "bg-[#1C2033] border-[#2A2D3E]"}`}>
      <svg width="13" height="13" viewBox="0 0 512 512" fill="none" stroke="currentColor" strokeWidth="32" strokeLinejoin="round" className={whiteIcons ? "text-white" : "text-[#8C9DB6]"}>
        <path d="M256 48a208.06 208.06 0 0 1 208 208c0 114.87-93.13 208-208 208a208.06 208.06 0 0 1-208-208C48 141.13 141.13 48 256 48z" />
        <path d="M256 48c-38.39 0-70.52 86.13-70.52 208S217.61 464 256 464s70.52-86.13 70.52-208S294.39 48 256 48z" />
        <path d="M464 256H48" />
      </svg>
      <select
        value={i18n.language}
        onChange={(e) => changeLanguage(e.target.value)}
        className={`bg-transparent text-xs font-bold outline-none cursor-pointer ${whiteIcons ? "text-white" : "text-[#8C9DB6]"}`}
      >
        {languages.map((lang) => (
          <option
            key={lang.code}
    ...
    <button
      onClick={toggleTheme}
      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${whiteIcons ? "bg-transparent border-transparent text-white hover:text-white" : "bg-[#1C2033] border-[#2A2D3E] text-[#8C9DB6] hover:text-white"}`}
    >
      {theme === "dark"
        ? <Sun size={13} className={whiteIcons ? "text-white" : "text-[#8C9DB6]"} />
        : <Moon size={13} className={whiteIcons ? "text-white" : "text-[#8C9DB6]"} />}
      {theme === "dark"
        ? t("footer.light")
        : t("footer.dark")}
    </button>
...
bg-clip-text text-transparent opacity-90">
  LAB WARDS
       ...
       <div className="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 md:px-10">
        ...
        {/* ── MOBILE layout ── */}
        <div className="md:hidden flex flex-col items-center gap-7 pb-24">
          ...
          {/* Bottom bar */}
          <div className="relative w-full border-t border-[#1E2133] pt-5 flex flex-col items-center gap-4">
...
/>
              ...
              {/* Mobile watermark — behind lang/theme buttons */}
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-6 select-none pointer-events-none">
                <h2 className="text-[75px] font-black leading-none whitespace-nowrap
                  bg-gradient-to-b from-[#1E3A5F] via-[#1E3A5F66] to-[#76A3DC00]
                  bg-clip-text text-transparent opacity-90">
                  LAB WARDS
                </h2>
